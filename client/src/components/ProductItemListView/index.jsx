...
const ProductItem = (props) => {
  ...
  // 👇 precio principal y oldPrice SIEMPRE en BOB (fallback a .price/.oldPrice)
  ...
  const addToCart = (product, userId, quantity) => {
    ...
    const productItem = {
      ...
      // 👇 guardamos precio en BOB para todo el flujo
      price: priceBob,
      oldPrice: oldPriceBob,
      discount: product?.discount,
      quantity: quantity,
      subTotal: parseInt((priceBob || 0) * quantity),
  ...
  const minusQty = () => {
    ...
    if (quantity === 1) {
      ...
      const obj = {
        ...
        qty: quantity - 1,
        // 👇 subTotal en BOB
        subTotal: (priceBob || 0) * (quantity - 1)
      }
      ...
      bg-[rgba(0,0,0,0.7)] z-[60] p-3 gap-2">
        ...
        <div className="actions absolute top-[-20px] right-[5px] z-50 flex items-center gap-2 flex-col w-[50px] transition-all duration-300 group-hover:top-[15px] opacity-0 group-hover:opacity-100">
          ...
          <Button className="!w-[35px] !h-[35px] !min-w-[35px] !rounded-full !bg-white  text-black hover:!bg-primary hover:text-white group"
            onClick={() => {
              const url = `${window.location.origin}/product/${props?.item?._id}`;
              if (navigator.share) { navigator.share({ title: props?.item?.name, url }); }
              else { navigator.clipboard.writeText(url); }
            }}>
            <FiShare2 className="text-[16px] !text-black group-hover:text-white hover:!text-white" />
          </Button>
        ...
        <div className="mt-3 w-[180px]">
          {
            isAdded === false ?
              ...
              <Button className="btn-org btn-border btn-sm  !w-[195px] !h-[35px]"
