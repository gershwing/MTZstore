...
export const Sidebar = (props) => {
  ...
  const filtesData = () => {
    ...
    if (context?.searchData?.products?.length > 0) {
      ...
      postData(`/api/product/filters`, filters).then((res) => {
        ...
        props.setProductsData(payload);
        props.setIsLoading(false);
        props.setTotalPages(payload?.totalPages)
        window.scrollTo(0, 0);
      })
    }
  ...
  return (
    <aside className="sidebar py-3  lg:py-5 static lg:sticky top-[130px] z-[50] pr-0 lg:pr-5">
        ...
        <div className="box mt-4">
          ...
          <div className="flex pt-4 pb-2 priceRange">
            <span className="text-[13px]">
              Desde: <strong className="text-dark">Bs: {price[0]}</strong>
            </span>
            <span className="ml-auto text-[13px]">
              Desde: <strong className="text-dark">Bs: {price[1]}</strong>
            </span>
          </div>
        </div>
      ...
      <br />
      <Button className="btn-org w-full !flex lg:!hidden" onClick={() => context?.setOpenFilter(false)}><MdOutlineFilterAlt size={20} /> Filtros</Button>
