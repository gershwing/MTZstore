...
// 👇 nuevo: formateador de precio BOB
...
const ProductItem = (props) => {
  ...
  // 👇 precio principal SIEMPRE en BOB (fallback a .price si aún no llega .priceBob)
  const priceBob =
    props?.item?.priceBob != null ? props.item.priceBob : props?.item?.price;

  const oldPriceBob =
    props?.item?.oldPriceBob != null
      ? props.item.oldPriceBob
      : props?.item?.oldPrice;
  ...
  const addToCart = (product, userId, quantity) => {
    const productItem = {
      ...
      // 👇 guardamos precio en BOB para todo el flujo del carrito
      price: priceBob,
  ...
  const handleAddToMyList = (item) => {
    if (context?.userData === null) {
      ...
      const obj = {
        ...
        image: item?.images?.[0],
        rating: item?.rating,
        // 👇 guardamos referencia de precio en BOB
        price: priceBob,
        oldPrice: oldPriceBob,
        brand: item?.brand,
