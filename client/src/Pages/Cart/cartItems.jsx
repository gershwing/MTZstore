...
const CartItems = (props) => {
  ...
  const numbers = Array.from({ length: 20 }, () => Math.floor(Math.random() * 10) + 1); // (no usado, lo dejo para no romper nada)

  const context = useContext(MyContext);

  // Precio principal SIEMPRE en BOB (fallback si aún no llega del backend)
  const priceBob = props?.item?.priceBob != null ? props.item.priceBob : props?.item?.price;
  const oldPriceBob = props?.item?.oldPriceBob != null ? props.item.oldPriceBob : props?.item?.oldPrice;
  ...
  const handleCloseQty = (value) => {
    ...
    if (value !== null) {
      ...
      const cartObj = {
        _id: props?.item?._id,
        qty: value,
        // subTotal SIEMPRE en BOB
        subTotal: (priceBob || 0) * value,
      };
  ...
  const updateCart = (selectedVal, qty, field) => {
    ...
    if (field === "weight") {
      fetchDataFromApi(`/api/product/${props?.item?.productId}`).then((res) => {
        const product = res?.product;
        const item = product?.productWeight?.filter((weight) => weight?.includes(selectedVal));
        if (item?.length !== 0) {
          editData("/api/cart/update-qty", cartObj).then((res) => {
            if (res?.data?.error === false) {
              context.alertBox("success", res?.data?.message);
    ...
    if (field === "ram") {
      fetchDataFromApi(`/api/product/${props?.item?.productId}`).then((res) => {
        const product = res?.product;
        const item = product?.productRam?.filter((ram) => ram?.includes(selectedVal));
        if (item?.length !== 0) {
          editData("/api/cart/update-qty", cartObj).then((res) => {
            if (res?.data?.error === false) {
              context.alertBox("success", res?.data?.message);
  ...
  return (
    <div className="cartItem w-full p-3 flex items-center gap-4 pb-5 border-b border-[rgba(0,0,0,0.1)]">
      ...
      <div className="info  w-[70%]  sm:w-[80%]  lg:w-[85%] relative">
        ...
        {props?.item?.variantAttrs && Object.keys(props.item.variantAttrs).length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1">
            {Object.entries(props.item.variantAttrs).map(([key, val]) => (
              <span key={key} className="text-[11px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded">
                {val}
              </span>
            ))}
          </div>
       ...
       font-[600] py-1 px-2 rounded-md cursor-pointer"
                    onClick={handleClickSize}
                  >
                    Peso: {selectedSize} <GoTriangleDown />
                  </span>
     ...
     font-[600] py-1 px-2 rounded-md cursor-pointer"
        ...
        <div className="flex items-center gap-4 mt-2">
          ...
          <span className="oldPrice line-through text-gray-500 text-[14px] font-[500]">
            {oldPriceBob ? formatPrice(oldPriceBob, "BOB") : ""}
          </span>
