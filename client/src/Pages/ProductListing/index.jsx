...
const ProductListing = () => {
  ...
  const [selectedSortVal, setSelectedSortVal] = useState("Nombre, A a Z");
  ...
  // 🟢 Mapear "price" → "priceBob" para ordenar por BOB
  const SORT_KEY_MAP = {
    price: "priceBob",
    name: "name",
  };
  ...
  const handleSortBy = (name, order, products, value) => {
    ...
    postData(`/api/product/sort`, {
      products: products,
      sortBy: sortKey, // <- usar priceBob cuando pidan "price"
      order: order,
    }).then((res) => {
      setProductsData(res);
      setAnchorEl(null);
    ...
    <section className=" pb-0">
      <div className="bg-white p-2">
        <div className="container flex gap-3">
          {/* 🩹 Mini fix de clase: 'w-fulllg:h-full' → 'w-full lg:h-full' */}
          <div
            className={`sidebarWrapper fixed -bottom-[100%] left-0 w-full lg:h-full lg:static lg:w-[20%] bg-white z-[102] lg:z-[100] p-3 lg:p-0 transition-all lg:opacity-100 opacity-0 ${context?.openFilter === true ? "open" : ""
              }`}
          >
            <Sidebar
            ...
            <div className="bg-[#f1f1f1] p-2 w-full mb-4 rounded-md flex items-center justify-between sticky top-[135px] z-[99]">
              <div className="col1 flex items-center itemViewActions">
                ...
                <span className="text-[14px] hidden sm:block md:block lg:block font-[500] pl-3 text-[rgba(0,0,0,0.7)]">
                  Hay {productsData?.products?.length || 0} productos.
                </span>
              </div>
              ...
              <div className="col2 ml-auto flex items-center justify-end gap-3 pr-4">
                ...
                >
                  <MenuItem
                    onClick={() =>
                      handleSortBy("name", "asc", productsData, "Nombre, A a Z")
                    }
                  ...
                  <MenuItem
                    onClick={() =>
                      handleSortBy("name", "desc", productsData, "Nombre, Z a A")
                    }
                    className="!text-[13px] !text-[#000] !capitalize"
                  >
                    Nombre, Z a A
                  ...
                  <MenuItem
                    onClick={() =>
                      handleSortBy("price", "asc", productsData, "Precio, de menor a mayor")
                    }
                    className="!text-[13px] !text-[#000] !capitalize"
                  >
                  ...
                  <MenuItem
                    onClick={() =>
                      handleSortBy("price", "desc", productsData, "Precio, de mayor a menor")
                    }
                    className="!text-[13px] !text-[#000] !capitalize"
                  >
